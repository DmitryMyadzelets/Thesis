...
parts which can be affected mechanically by the material:
\begin{itemize}
...
detection. If not a particular single fault can not be detected, at least one
...
\section{How to Deal with Failures?}
...
\subsection{Prevent, Detect, Isolated and Recover}
...
This problem is addressed with the knowledge based approach and analytical
methods at the design stage while the development of a new system.
Assuming that any failure which was determined can occur, the next step is to
figure out what negative impact of each failure (of type of failures) is
necessary to detect, ans which is not. The failure detection may be an expensive
process, in terms of time, increasing complexity of the system, and costs.
One of the most exhaustive approaches addressing the task to determine failures
and to evaluate them is the Failure Mode and Effects Analysis (FMEA). Another
technique which can help uncover potential problems is a simulation process.
...
The problem of fault-tolerance is out the scope of this work. We proceed
with a brief description of the FMEA as a pure industrial approach, simulation
process which is widely used in both industrial and academic fields, and formal
methods as an academic response to the problem of failures diagnosis.
...
from happening. FMEA is described in the {IEC 60812} standard, and consists of:
\begin{itemize}
...
less costly but yet knowledge based analytical approaches, presented in
literature, for instance the adapted for industrial use Process Mapping
\cite{what_is_a_process_map}, and State analyses
\cite{ruiz_state_2000}
...
be dangerous to use its processes and machines. Simulation is also used in
performance optimisation \cite{tamimi_simulation_2012} and scientific modeling
\cite{sharda_best_2011}. The main issue of this approach is how an imitated
system is close to the model, i.e. how quality and quantity of information
provided for simulation reflects the processes and machines' behaviour in the
real life.


\subsection{Formal methods}

The above described Failure Mode and Effect Analysis the first step of a
system reliability study. The formal methods augment \ldots

Formal methods
What formal methods are

. Simulation (it is not a formal method)
. Emulation (that is out of this topic)
. Validation
. Verification
\ldots
