...
\chapter{Modeling and Verification Framework}
...
moment of rising a temperature above a certain level. The correspondent states
...
The next section describes a modeling approach of applying design patterns for
functional design of DES and verification of system's properties. The approach
...
particularly the legacy IEC 61131-3 \cite{otto_iec_2009} which defines
programming languages for programmable logic controllers (PLC), and the modern
IEC 61499 \cite{vyatkin_iec_2009} which represents a component solution for
distributed industrial automation systems aiming at portability, reusability,
...
development, enrollment and maintenance of a complex system. They
exploit object-oriented concepts as classes with their instances, inheritance
and encapsulation. However, these concepts were not initially created bearing in
mind the purpose of formal verification. As a consequence, the tools widely used
in industry nowadays do not generally support approaches for DES raising in the
scientific world. As a response for the growing demand for intersection of
classical development process with formal techniques many researches
attempt to create instrumental and theoretical bridges. The most
promising results seem to be reached in formalizing Unified Modeling Language
(UML), Statecharts, and general purpose programming languages as C. 
...
% \draw[help lines] (0,0) grid (6,2);
	...
	\caption{Example of an UML diagram (at top) and its representation by
	the Mealy automaton}
...
\begin{figure}[!t]
	...
	(the image is borrowed from \cite{faldella_hierarchical_2008})}
	\label{fig:ga_blocks}
\end{figure}
...
\ref{fig:ga_blocks}. Besides the essential inputs and outputs necessary to
perform the above described types of actions, the blocks have other inputs and
...
correspondent to the GAs can became very specific, such that it would be
...
two sensors each of them gives the feedback for the correspondent state. In case
...
\begin{figure}[t]
	\centering
	\includegraphics[width=30em]{gd_sadf_automaton.png}
	\caption{An automaton representing single acting generalized device with
	double feedback (the image is borrowed from \cite{sartini_architectures_2010})}
	\label{fig:gd_sadf_automaton}
\end{figure}

As an instance, the Figure \ref{fig:gd_sadf_automaton} depicts an automaton
which represents the single acting generalized device with double feedback.

The approach of generalized actuators and devices allows to construct a complex
automated system using simple predefined models. Due to the representation of
the system's models by automata, various DES techniques can be
applied in order to define and solve control and diagnosis problems. The next
section proceeds with a description of how the approach of generalized device is
used for modeling and analyses of faults.


\subsubsection{Generalized Device and Fault Diagnosis}

This section briefly describes an approach for modular module-building of
complex systems, presented in \cite{sartini_methodology_2010}. The methodology
extends the approach of generalized device towards the better reusability, and
suitability for formal verification techniques. However, it concentrates on
modeling aspects rather then verification procedures. 

Bearing in mind that the high-level modeling was covered by the General
Actuator concept, the low-level modeling  \ldots

\emph{Here goes the general description of the low-level modeling (individual
components, physical coupling, control policies), and then the fault modeling
part.}

% Models of:
% individual hardware components
% physical coupling
% control logic (policy) = control actions of high level (manages its sequence)
