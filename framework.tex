...
simulation, using existing tools aimed for this purpose. In
\cite{bonfe_design_2003} the authors develop a framework to transform UML
diagrams and Statecharts into Computation Tree Logic (CTL) which is the used in
...
\cite{ray_automated_2007} and \cite{li_stateflow_2011}. As a model checking
tool, which many of the above mentioned approaches exploit, the most notable one
is NuSMV \cite{nusmv} which automatically verifies properties of finite-state
systems, expressed in CTL, Linear Temporal Logic (LTL) or Property Specification
Language (PSL).
...
\begin{figure}[!t]
	...
	\caption{Example of UML diagram}
	\label{fig:example_of_UML}
\end{figure}
