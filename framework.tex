...
programmer or developed at the level of international standard committees and
organisations. As instances, one can recall object-oriented concepts in
...
formal verification is presented in \cite{sartini_architectures_2010}, which
introduces a modeling entity called \emph{Generalized Device} and shows how it
can be used for formal verification in the DES framework. The approach is a
...
breve introduction of this concept and the refined approach.


\subsection{Generalized Device Abstraction}

The architectural approach of Generalized Device (GD) is based on a modeling
framework with Generalized Actuator (GA), presented in
\cite{faldella_hierarchical_2008}. The GA approach aims to reduce complexity of
the modeling process using standardized object-oriented concepts of engineering.
...
instance of a process which can be abstracted by this type of GA is opening a
valve: a command ``Do'' to open the valve is issued; when it opens an event
``Done'' occurs.
...
a temperature regulation with a heater: ``Start'' event switches on the heater;
when temperature reaches a required level the ``Stop'' event switches it off.
...
\ldots Continue with description of the automaton from the picture.
Modeling steps and etc.\ldots
