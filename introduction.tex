...
Modern society is characterized by the growing demand for usage of automated
systems in the everyday life, while the systems require software engineering
with constantly increasing complexity. Industrial automation software
engineering was traditionally a field having the most strict requirements and
high standards applied. However, it is still a common practice when an
automation software design consists of writing a ladder logic for programmable
...
cost in the costs of machinery, engineers and researches put a lot of efforts to
...
a growing number of formal methods - mathematical approaches supporting
...
There are hundreds major producers of automation hardware and software, but the
variety of ways the developers can write programs is limited by the standards,
such as {IEC 61131} making it less error prone and more reusable. In other
...
aspect-oriented programming, software product line and etc (see
...
reliability and other important properties of systems are mainly the concern of
...
notations requires to have additional knowledge for development engineers,
creates psychological barrier to them, and that the 'de facto' development
process does not incorporate a formal representation. 
...
a non formal (or semi-formal) system's representation has the ability to be
...
properties should be expressed in a formal way again by engineers, which rises
the above mentioned problems. Thus, despite of the fact that some successful
reports can be found in literature, the progress in this way seems low
...
and the formal techniques. Nowadays, when the growing complexity of automated
systems imposes requirements which can be ensured only by formal methods, the
relevant response may be the creation of modeling techniques when the models
...
that the correspondent computational burden remains at an acceptable level.
...
\begin{itemize}
  ...
  formulation is used then to enforce the desired property of the system, rather
  then just verifying it. This approach tackles the state explosion problem
  ...
  allows to embed  formal approach into a higher level of the systems'
  representation of ``standard'' modular design process, similar to one
...
fault diagnosis on a real example of the industrial automation plant. It
