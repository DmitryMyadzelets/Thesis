...
\chapter{Modular Virtual Diagnosability}

We define our goal as to make the system modularly diagnosable. If we assume
that the global system is diagnosable with respect to a given fault, then it
implies that it is always possible to find a subset of the modules, such
that it contains a faulty module, and the modules can be composed into
one locally diagnosable module. Each module in this subset helps to
detect and distinguish the fault by its sequence of observable events. It also
implies that not each module of the system can or may participate in the fault
detection and distinguishing process. We proceed with the formal definition of
modular virtual diagnosability.
...
partition is locally diagnosable. The total number of partitions of a set with
cardinality $n$ is known as the Bell number $B_n$, which has a double
exponential generation function \cite{brualdi_introductory_2004}. In
reality, it may not necessary to enumerate all the partitions, since a
partition which corresponds to modularly diagnosable system can be found
event at the first iteration. Moreover, it is not necessary to compose all
the modules in each element of the partition, since not all them elements
may contain modules with faults. However, the level of complexity in the worst
case motivates us to find a more smart solution for the problem. 

In the next section we make a first step toward creation of a feasible approach
for the problem.


\section{Trivial system. Analysis of two adjacent modules}

As was mentioned above, the first step to improve the search for relevant
partition, would be to enumerate only the modules which potentially can
influence diagnosability. Intuitively, those are ones which have observable
events. Then, the structure of a module may have some patterns which could be
found before the module is composed with others. The module with fault may also
be analyzed for a possibility to change its observation due to interaction with
other modules. Thus, in the trivial case of a system consisting of two
modules, where only one module has a fault, in order to solve the problem we
need to answer the following questions: a) can the module with a fault
potentially change the observation of its strings while composed with adjacent
module? and b) can the module change observation some strings of another module
due to concurrency? No parallel composition should be used in order to answer
the questions. Otherwise, the problem would reduce to the problem of local
diagnosability.

We suppose that the system consists only of two modules with the correspondent
...
\section{General form of conditions}
...
Observation propagation algorithm

Multiple choice for a virtual module
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Construction of a virtual module}
